...
<div class="min-h-screen bg-gray-50 dark:bg-gray-900">
    ...
    <div class="bg-white dark:bg-gray-800 shadow">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
            <div class="flex items-center justify-between">
                <div>
                    <h1 class="text-2xl font-bold text-gray-900 dark:text-white">Edit Doctor</h1>
                    <p class="text-gray-600 dark:text-gray-400">Update doctor information</p>
                </div>
    ...
    <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div class="bg-white dark:bg-gray-800 rounded-lg shadow">
            <div class="p-6">
                <form id="editDoctorForm" method="POST" action="{{ route('admin.doctors.update', $doctor->id) }}">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="mb-8">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div>
                                ...
                                <input type="text" id="education" name="education"
                                       class="w-full px-4 py-3 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-gold focus:border-transparent bg-white dark:bg-gray-800 text-gray-900 dark:text-white"
                                       value="{{ old('education', $doctor->education) }}" placeholder="e.g., MBBS, MD, PhD">
                                @error('education')
                                    <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                                @enderror
                            </div>
                            ...
                            <div>
                                ...
                                <input type="text" id="languages" name="languages"
                                       class="w-full px-4 py-3 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-gold focus:border-transparent bg-white dark:bg-gray-800 text-gray-900 dark:text-white"
                                       value="{{ old('languages', $doctor->languages) }}" placeholder="e.g., English, Arabic, French">
                                @error('languages')
                                    <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                                @enderror
                            </div>
                        ...
                        <div class="mt-6">
                            ...
                            <textarea id="description" name="description" rows="4"
                                      class="w-full px-4 py-3 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-gold focus:border-transparent bg-white dark:bg-gray-800 text-gray-900 dark:text-white"
                                      placeholder="Brief description about the doctor's expertise and experience">{{ old('description', $doctor->description) }}</textarea>
                            @error('description')
                                <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                            @enderror
                        </div>
    ...
    <div class="flex items-center justify-center min-h-screen">
        <div class="bg-white dark:bg-gray-800 rounded-lg p-6">
            <div class="flex items-center space-x-3">
                ...
                <span class="text-gray-900 dark:text-white">Updating doctor...</span>
            </div>
        </div>
    </div>
</div>
...
document.getElementById('editDoctorForm').addEventListener('submit', function(e) {
    ...
    .then(data => {
        ...
        if (data.success) {
            ...
            setTimeout(() => {
                window.location.href = '{{ route("admin.doctors.show", $doctor->id) }}';
            }, 1500);
        } else {
            if (data.errors) {
                ...
                showNotification(data.message || 'Error updating doctor', 'error');
            }
        }
    })
    .catch(error => {
