...
<div class="min-h-screen bg-surface">
    ...
    <div class="bg-gradient-to-r from-gold/10 to-gold-deep/10 py-8">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="flex items-center justify-between">
                <div>
                    <h1 class="text-3xl font-bold text-text">Edit User</h1>
                    <p class="text-muted mt-2">Modify user information</p>
                </div>
                <div class="flex items-center space-x-4">
                    ...
                    <a href="{{ route('admin.users.show', $user->id) }}" class="btn btn-outline">
    ...
    <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div class="card feature-card">
            <div class="p-6">
                <form id="userForm" method="POST" action="{{ route('admin.users.update', $user->id) }}" class="space-y-6">
                    @csrf
                    @method('PUT')
                    ...
                    <div id="doctorFields" class="{{ $user->role === 'doctor' ? '' : 'hidden' }}">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                                ...
                                <select id="specialty_id" name="specialty_id"
                                    ...
                                    @foreach($specialties as $specialty)
                                        <option value="{{ $specialty->id }}" {{ $user->specialty_id == $specialty->id ? 'selected' : '' }}>
                                            {{ $specialty->name_en }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div class="md:col-span-2">
                                ...
                                <textarea id="education" name="education" rows="3"
                                    class="w-full px-4 py-3 rounded-lg border border-gray-300 focus:ring-2 focus:ring-gold focus:border-transparent bg-white text-text"
                                    placeholder="Enter education details">{{ $user->education }}</textarea>
                            </div>
                            ...
                            <div class="md:col-span-2">
                                ...
                                <textarea id="description" name="description" rows="3"
                                    class="w-full px-4 py-3 rounded-lg border border-gray-300 focus:ring-2 focus:ring-gold focus:border-transparent bg-white text-text"
                                    placeholder="Enter professional description">{{ $user->description }}</textarea>
                            </div>
                        </div>
                    </div>

                    <!-- Patient Information (Conditional) -->
                    <div id="patientFields" class="{{ $user->role === 'patient' ? '' : 'hidden' }}">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div class="md:col-span-2">
                                ...
                                <textarea id="medical_history" name="medical_history" rows="3"
                                    class="w-full px-4 py-3 rounded-lg border border-gray-300 focus:ring-2 focus:ring-gold focus:border-transparent bg-white text-text"
                                    placeholder="Enter medical history">{{ $user->medical_history }}</textarea>
                            </div>
                        </div>
                    </div>
